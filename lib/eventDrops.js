...
function eventDrops(config = {}) {
    ...
    function eventDropGraph(selection) {
        return selection.each(function selector(data) {
            ...
            this.draw = drawer(svg, dimensions, scales, finalConfiguration).bind(selection);
            this.draw(data);

            if (finalConfiguration.zoomable) {
                this.zoom = zoom(d3.select(this), dimensions, scales, finalConfiguration, data, this.draw);
            }
        });
    }
